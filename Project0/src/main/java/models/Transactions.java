...
public class Transactions {
	...
	public String toPrettyString() {
		...
		String source_customer_name = CustomerServiceImpl.getInstance().getCustomer(this.source.getCustomerId())
				.getUsername();
		...
		String str = String.format("Transaction %3d -> (%d) %s: %10s | %-10s of %10s", this.id, source_id,
				source_customer_name, source_balance, this.type, amount_format);

		StringBuilder builder = new StringBuilder(str);

		if (this.type.equalsIgnoreCase("transfer")) {
			int receiver_id = this.receiver.getId();
			String receiver_customer_name = CustomerServiceImpl.getInstance().getCustomer(this.receiver.getCustomerId())
					.getUsername();
