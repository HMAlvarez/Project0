...
//		while (true) {
			...
			switch (command[0]) {
				case "1":
					...
					AccountServiceImpl.getInstance().apply(amount);
				...
				case "2":
					...
					success = AccountServiceImpl.getInstance().withdraw(fromID, amount);
					if (success) {
				...
				case "3":
					...
					amount = Float.parseFloat(command[2]);
					success = AccountServiceImpl.getInstance().deposit(fromID, amount);
					if (success) {
				...
				case "4":
					...
					success = AccountServiceImpl.getInstance().transfer(fromID, toID, amount);
					if (success) {
	...
	private static void handleEmployeeOptions(String[] command) {
		...
		switch (command[0]) {
			...
			case "6":
				...
				Customer c = CustomerServiceImpl.getInstance().getCustomer(Integer.parseInt(command[1]));
				printAccounts(c);
				break;
			case "7":
